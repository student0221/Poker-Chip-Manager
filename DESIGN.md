...
| 功能 | 状态 | 说明 |
|------|------|------|
| 阶段 1：基础局域网 MVP | ✅ 已完成 | 后端监听 `0.0.0.0`，`GET /api/network-info` 返回局域网 IP |
| 阶段 2：多房间支持 | ✅ 已完成 | rooms 表 + players.room_id，`/api/rooms/:roomId/...` 系列路由 |
| 阶段 3：实时同步 | ✅ 已完成 | Socket.io 事件广播，客户端收到事件后重新拉取 HTTP 状态 |
| 阶段 4：局域网发现 | ✅ 已完成 | Node UDP 广播/监听 + `GET /api/discovered-hosts` |
| 双轨兼容 | ✅ 已完成 | 旧 API 映射 default 房间，旧页面 `/#/`、`/#/admin` 继续可用 |
| 移动端适配 | ✅ 已完成 | 缩放控制、横屏适配、操作面板位置优化（2026-06-07） |
| SQLite WAL模式 | ✅ 已完成 | `PRAGMA journal_mode = WAL`，`busy_timeout = 5000` |
| 事务保障 | ✅ 已完成 | 结算、重置、解散使用事务，批量更新 players net_profit |
...
| 原有 `/#/` 玩家页仍可完成报名、离场、提交最终筹码 | ✅ |
| 原有 `/#/admin` 管理页仍可开始、结束、补筹码、清算、重置 | ✅ |
| 现有 `tests/api.test.js` 通过 | ✅（2026-06-07 修复 post-rebase 测试） |
| 旧 API 和新房间 API 同时存在，不互相覆盖 | ✅ |
| 历史数据库升级后，旧玩家数据仍能在默认房间中看到 | ✅ |
| 同一 WiFi 下至少 3 台设备可通过二维码或 IP 访问主机 | ✅ |
| 手机端可完成报名、离场、提交最终筹码 | ✅ |
| 管理端可开始、结束、清算一场比赛 | ✅ |
| 页面展示局域网访问地址和二维码 | ✅ |
| 可创建 2 个以上房间 | ✅ |
...
| 同一昵称可出现在不同房间，但不能在同一房间重复 | ✅ |
| 非房主设备不能执行开始、结束、清算、重置、解散 | ✅ |
| 报名、离场、补筹码、状态变化能在 500ms 内同步 | ✅ |
| 清算完成后所有设备能看到一致 rankings | ✅ |
| Socket 断开重连后能重新拉取最新房间状态 | ✅ |
| 后端可发现同局域网内其他主机广播的房间 | ✅ |
| 自动发现失败时，IP/二维码加入仍然可用 | ✅ |
...
| 功能 | 说明 |
|------|------|
| 房主断线自动转移 | 当前版本不自动处理，推荐房主设备重新进入继续管理 |
| 公网联机 | 当前仅支持同一 WiFi/局域网，跨公网方案待设计 |
| mDNS/Bonjour 自动发现 | 作为 UDP 的增强路径，需进一步兼容性测试 |
...
```
2026-06-07  fix post-rebase database and admin tests
2026-06-07  add mobile zoom levels and self marker
2026-06-07  fix mobile action controls and dock position
2026-06-07  fix mobile landscape action panel visibility
2026-06-07  add mobile table zoom controls
```
